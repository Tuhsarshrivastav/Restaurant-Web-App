...
const Special = require("../models/addSpecial");
...
router.get("/getSpacial", async (req, res) => {
  try {
    const pizzas = await Special.find({});
    res.send(pizzas);
  } catch (error) {
    res.json({ message: error });
  }
...
router.post("/getspecialbyid", async (req, res) => {
  const pizzaId = req.body.pizzaId;
  try {
    const pizza = await Special.findOne({ _id: pizzaId });
    res.send(pizza);
  } catch (error) {
    res.json({ message: error });
  }
...
router.post("/updatespacial", async (req, res) => {
  const updatedPizza = req.body.updatedPizza;
  try {
    const pizza = await Special.findOne({ _id: updatedPizza._id });
    (pizza.name = updatedPizza.name),
      (pizza.description = updatedPizza.description),
      (pizza.image = updatedPizza.image),
      (pizza.category = updatedPizza.category),
